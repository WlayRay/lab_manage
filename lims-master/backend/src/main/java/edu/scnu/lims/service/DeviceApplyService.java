...
public class DeviceApplyService {
    ...
    public List<DeviceApply> getDeviceAppliesByUserIdOrLaboratoryId(int page, int pageSize, Integer userId,
            Integer laboratoryId) {
        return deviceApplyDao
                .findByUser_UserIdOrDevice_Laboratory_LaboratoryId(userId, laboratoryId, PageRequest.of(page, pageSize))
                .toList();
    }
    ...
    public DeviceApply updateApply(DeviceApply deviceApply, DeviceApplyStatusEnum status, UserRoleEnum role) {
        switch (role) {
            case SUPER_ADMIN:
                if (status == DeviceApplyStatusEnum.CANCELED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.APPLIED) {
                    deviceApply.setStatus(status);
                    // 恢复设备状态
                    Device device = deviceApply.getDevice();
                    device.setStatus(DeviceStatusEnum.AVAILABLE);
                    deviceService.saveDevice(device);
                } else if (status == DeviceApplyStatusEnum.GRANTED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.APPLIED) {
                    deviceApply.setStatus(status);
                    deviceApply.setGrantTimestamp(System.currentTimeMillis());
                } else if (status == DeviceApplyStatusEnum.DENIED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.APPLIED) {
                    deviceApply.setStatus(status);
                    // 恢复设备状态
                    Device device = deviceApply.getDevice();
                    device.setStatus(DeviceStatusEnum.AVAILABLE);
                    deviceService.saveDevice(device);
                } else if (status == DeviceApplyStatusEnum.RETURNED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.GRANTED) {
                    deviceApply.setStatus(status);
                    deviceApply.setReturnTimestamp(System.currentTimeMillis());
                } else if (status == DeviceApplyStatusEnum.FINISHED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.RETURNED) {
                    deviceApply.setStatus(status);
                ...
                break;
            case ADMIN:
                if (status == DeviceApplyStatusEnum.GRANTED
                        && deviceApply.getStatus() == DeviceApplyStatusEnum.APPLIED) {
                    deviceApply.setStatus(status);
                    deviceApply.setGrantTimestamp(System.currentTimeMillis());
                } else if (status == DeviceApplyStatusEnum.DENIED
